$PBExportHeader$w_controls_multas_mat.srw
$PBExportComments$Permite cargar el pago de  las multas del material
forward
global type w_controls_multas_mat from w_sheet
end type
type st_5 from statictext within w_controls_multas_mat
end type
type cbx_intercambio from checkbox within w_controls_multas_mat
end type
type st_4 from statictext within w_controls_multas_mat
end type
type st_3 from statictext within w_controls_multas_mat
end type
type st_2 from statictext within w_controls_multas_mat
end type
type st_1 from statictext within w_controls_multas_mat
end type
type em_operacion from editmask within w_controls_multas_mat
end type
type em_corte from editmask within w_controls_multas_mat
end type
type em_caja from editmask within w_controls_multas_mat
end type
type dw_solicitud from u_dw within w_controls_multas_mat
end type
type sle_cuenta from singlelineedit within w_controls_multas_mat
end type
type rb_2 from radiobutton within w_controls_multas_mat
end type
type rb_1 from radiobutton within w_controls_multas_mat
end type
type cb_cerrar from commandbutton within w_controls_multas_mat
end type
type cb_actualiza from commandbutton within w_controls_multas_mat
end type
type dw_interesado from u_dw within w_controls_multas_mat
end type
type gb_1 from groupbox within w_controls_multas_mat
end type
type gb_2 from groupbox within w_controls_multas_mat
end type
end forward
...
global w_controls_multas_mat w_controls_multas_mat
...
on w_controls_multas_mat.create
int iCurrent
call super::create
...
on w_controls_multas_mat.destroy
call super::destroy
destroy(this.st_5)
...
integer height = 472
integer taborder = 30
string dataobject = "d_controls_mat_multa"
boolean border = false
end type
		...
		if dw_solicitud.update()>0 then
			...
			for llRow = 1 to dw_solicitud.RowCount()
				...
				choose case llEstatus
					case 1
//						llFolio = dw_solicitud.getitemnumber(llRow,'spre_multas_folio')
						...
						liCaja = integer(em_caja.text)
						liCorte = integer(em_corte.text)
						liOper = integer(em_operacion.text)
						...
						SET fcaja = :liCaja, foperacion =:liOper, fcorte = :liCorte
						WHERE ( dbo.spre_multas.folio = :llFolioDat ) AND
								( dbo.spre_multas.cvedepto = 5)
						using gtr_sumuia;	
...
integer height = 184
integer taborder = 30
string dataobject = "d_psi_info_multa"
boolean border = false
end type
