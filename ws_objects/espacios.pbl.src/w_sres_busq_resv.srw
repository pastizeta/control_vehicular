$PBExportHeader$w_sres_busq_resv.srw
forward
global type w_sres_busq_resv from window
end type
type cb_salir from commandbutton within w_sres_busq_resv
end type
type cb_buscar from commandbutton within w_sres_busq_resv
end type
type dw_criterios from datawindow within w_sres_busq_resv
end type
type gb_1 from groupbox within w_sres_busq_resv
end type
type dw_busqueda from datawindow within w_sres_busq_resv
end type
type mle_1 from multilineedit within w_sres_busq_resv
end type
end forward
...
global w_sres_busq_resv w_sres_busq_resv
...
string ls_hrini='00:00:00', ls_hrfin='23:59:59'
...
if not isnull(dw_criterios.getitemnumber(1,'tipo_solicitud')) then
		ls_criterio+=" AND ( spre_solicitud.tipo_solicitud="+string(dw_criterios.getitemnumber(1,'tipo_solicitud'))+" ) "
end if
...
if not isnull(dw_criterios.getitemnumber(1,'requerimientos')) then
		ls_criterio+=" AND ( spre_equipo_solicitud.cveequipo="+string(dw_criterios.getitemnumber(1,'requerimientos'))+" ) "
end if

if not isnull(dw_criterios.getitemnumber(1,'tipo_nombre')) then
	
	CHOOSE CASE dw_criterios.getitemnumber(1,'tipo_nombre')
	CASE 1
		ls_criterio+=" AND ( upper( spre_reserva_espacios.sol_nombre ) like '%"+dw_criterios.getitemstring(1,'nombres')+"%' ) "
	CASE 2
		ls_criterio+=" AND ( upper( spre_reserva_espacios.resp_nombre ) like '%"+dw_criterios.getitemstring(1,'nombres')+"%' ) "
	CASE 3
		ls_criterio+=" AND ( upper( spre_reserva_espacios.aval_nombre ) like '%"+dw_criterios.getitemstring(1,'nombres')+"%' ) "
END CHOOSE
		
end if
if not isnull(dw_criterios.getitemstring(1,'evento')) then
		ls_criterio+=" AND ( upper(spre_reserva_espacios.nombre_evento) like '%"+string(dw_criterios.getitemstring(1,'evento'))+"%' ) "
end if
if not isnull(dw_criterios.getitemstring(1,'impartido')) then
		ls_criterio+=" AND ( upper(spre_reserva_espacios.impartido_por) like '%"+string(dw_criterios.getitemstring(1,'impartido'))+"%' ) "
end if
...
if not isnull(dw_criterios.getitemnumber(1,'horarioini')) or not isnull(dw_criterios.getitemnumber(1,'horariofin')) then
	if dw_criterios.getitemnumber(1,'horarioini')<dw_criterios.getitemnumber(1,'horariofin') then		
		...
		ls_hrfin=string(dw_criterios.getitemnumber(1,'horariofin'))+":59:59"
	else
...
li_tipo_espacio=dw_criterios.getitemnumber(1,'tipo_solicitud')
li_tipo_espacio=dw_criterios.getitemnumber(1,'folio_solicitud')
li_tipo_espacio=dw_criterios.getitemnumber(1,'tipo_espacio')
...
ls_evento=dw_criterios.getitemstring(1,'evento')

dw_busqueda.dataobject='d_sres_busq_resv1'
dw_busqueda.settransobject(gtr_sumuia)
...
integer taborder = 20
string title = "none"
string dataobject = "d_sres_criterios_busq_resv"
...
string dataobject = "d_sres_busq_resv1"
