$PBExportHeader$w_csal_prestamo_cantidad.srw
forward
global type w_csal_prestamo_cantidad from window
end type
type cb_guardar from commandbutton within w_csal_prestamo_cantidad
end type
type st_1 from statictext within w_csal_prestamo_cantidad
end type
type sle_cantidad from singlelineedit within w_csal_prestamo_cantidad
end type
end forward
...
global w_csal_prestamo_cantidad w_csal_prestamo_cantidad

type variables
integer ii_cantidad=0, ii_maximo
end variables

on w_csal_prestamo_cantidad.create
...
on w_csal_prestamo_cantidad.destroy
...
if isnumber(trim(sle_cantidad.text)) then
	...
	if ii_cantidad<=ii_maximo then
		close(w_csal_prestamo_cantidad)
	else
		ii_cantidad=0
