$PBExportHeader$w_veh_cat_choferes.srw
forward
global type w_veh_cat_choferes from window
end type
type dw_catalogo from datawindow within w_veh_cat_choferes
end type
type cb_nuevo from commandbutton within w_veh_cat_choferes
end type
type gb_titulo from groupbox within w_veh_cat_choferes
end type
end forward
...
string title = "Catálogo de choferes"
...
global w_veh_cat_choferes w_veh_cat_choferes
...
integer taborder = 10
string title = "none"
string dataobject = "d_veh_cat_choferes_edit"
...
if row>0 then
	if isvalid(w_veh_cat_choferes_edita) then close(w_veh_cat_choferes_edita) 
	ll_id=dw_catalogo.GetItemNumber(row,'id')
	openwithparm(w_veh_cat_choferes_edita,ll_id)
...
if isvalid(w_veh_cat_choferes_edita) then close(w_veh_cat_choferes_edita) 
openwithparm(w_veh_cat_choferes_edita,0)
li_respuesta=Message.DoubleParm
