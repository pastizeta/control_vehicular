$PBExportHeader$w_veh_cat_servicios.srw
forward
global type w_veh_cat_servicios from window
end type
type dw_catalogo from datawindow within w_veh_cat_servicios
end type
type cb_nuevo from commandbutton within w_veh_cat_servicios
end type
type gb_titulo from groupbox within w_veh_cat_servicios
end type
end forward
...
string title = "Catálogo de servicios y mantenimiento"
...
global w_veh_cat_servicios w_veh_cat_servicios
...
on w_veh_cat_servicios.destroy
...
integer taborder = 10
string title = "none"
string dataobject = "d_veh_cat_servicios_edit"
...
if row>0 then
	if isvalid(w_veh_cat_servicios_edita) then close(w_veh_cat_servicios_edita) 
	ll_id=dw_catalogo.GetItemNumber(row,'id')
	openwithparm(w_veh_cat_servicios_edita,ll_id)
...
if isvalid(w_veh_cat_servicios_edita) then close(w_veh_cat_servicios_edita)
openwithparm(w_veh_cat_servicios_edita,0)
li_respuesta=Message.DoubleParm
...
string facename = "Verdana"
long backcolor = 30520472
string text = "Servicios y mantenimientos"
end type
