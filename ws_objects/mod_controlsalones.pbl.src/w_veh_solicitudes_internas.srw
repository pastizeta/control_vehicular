$PBExportHeader$w_veh_solicitudes_internas.srw
forward
global type w_veh_solicitudes_internas from window
end type
type st_5 from statictext within w_veh_solicitudes_internas
end type
type em_fecha_inicio from editmask within w_veh_solicitudes_internas
end type
type dw_cat_estatus_solicitud from datawindow within w_veh_solicitudes_internas
end type
type cb_filtros from commandbutton within w_veh_solicitudes_internas
end type
type gb_filtros from groupbox within w_veh_solicitudes_internas
end type
type dw_solicitudes from datawindow within w_veh_solicitudes_internas
end type
type gb_titulo from groupbox within w_veh_solicitudes_internas
end type
type st_1 from statictext within w_veh_solicitudes_internas
end type
type sle_sol_id from singlelineedit within w_veh_solicitudes_internas
end type
type st_2 from statictext within w_veh_solicitudes_internas
end type
type sle_nombre_solicitante from singlelineedit within w_veh_solicitudes_internas
end type
type st_3 from statictext within w_veh_solicitudes_internas
end type
type sle_solicitante from singlelineedit within w_veh_solicitudes_internas
end type
type st_4 from statictext within w_veh_solicitudes_internas
end type
end forward
...
global w_veh_solicitudes_internas w_veh_solicitudes_internas
...
on w_veh_solicitudes_internas.create
...
on w_veh_solicitudes_internas.destroy
destroy(this.st_5)
...
// Actualizar con el estatus VENCIDA las solicitudes que tengan fecha final menor que el día de hoy y que tengan estatus 'Solicitada' ...
Update	via_solicitud_interna
Set		estatus = 7
Where	convert ( DATE , fecha_final ) < convert ( DATE , getdate () ) AND
			estatus = 0
Using		gtr_cv;

/*
// Actualizar con el estatus NO DISPONIBLE en el catalogo de vehiculos que tengan en la tabla de solicitudes la fecha inicial igual que el día de hoy y que tengan estatus 'Unidad Asignada' ...
Update	via_cat_vehiculos
Set		via_cat_vehiculos.estatus = 2
Where	id IN
			(
				SELECT	via_sol_vehiculos.id_vehiculo
				FROM		via_solicitud_interna,
							via_sol_vehiculos
				Where	convert ( DATE , via_solicitud_interna.fecha_inicio ) = convert ( DATE , getdate () ) AND
							via_solicitud_interna.estatus = 2 AND
							via_solicitud_interna.sol_id = via_sol_vehiculos.sol_id
...
//dw_solicitudes.Retrieve(790) // luego cambiar por 0 cph nov 2021
dw_solicitudes.Retrieve(0) // luego cambiar por 0 cph nov 2021
...
ELSE
	dw_solicitudes.SetFilter ( "String ( fecha_inicio , 'dd-mm-yyyy' ) = '" + Text + "'" )
END IF
...
integer taborder = 60
string title = "none"
string dataobject = "d_veh_solicitudes_edit"
...
if row>0 then
	...
	if isvalid(w_veh_solicitud_interna_edita) then close(w_veh_solicitud_interna_edita) 
	ll_id=this.GetItemNumber(row,'sol_id')
	openwithparm(w_veh_solicitud_interna_edita,ll_id)
...
string facename = "Verdana"
long backcolor = 30520472
string text = "Solicitudes internas de vehiculos"
end type
...
event ue_enchange;IF Text = "" THEN
	dw_solicitudes.SetFilter ( "" )
ELSE
	dw_solicitudes.SetFilter ( "Upper ( nombre_solicitante ) like '%" + Upper ( Text ) + "%'" )
END IF
...
event ue_enchange;IF Text = "" THEN
	dw_solicitudes.SetFilter ( "" )
ELSE
	dw_solicitudes.SetFilter ( "String ( solicitante ) like '%" + Text + "%'" )
END IF
