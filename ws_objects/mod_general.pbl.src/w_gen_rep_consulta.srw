$PBExportHeader$w_gen_rep_consulta.srw
$PBExportComments$Reportes reserva espacios comunicación
forward
global type w_gen_rep_consulta from pfc_w_sheet
end type
type rb_estatus from radiobutton within w_gen_rep_consulta
end type
type rb_vencido from radiobutton within w_gen_rep_consulta
end type
type rb_multa from radiobutton within w_gen_rep_consulta
end type
type cbx_esp from checkbox within w_gen_rep_consulta
end type
type cbx_mat from checkbox within w_gen_rep_consulta
end type
type cb_1 from commandbutton within w_gen_rep_consulta
end type
type cb_imprimir from commandbutton within w_gen_rep_consulta
end type
type cb_generar from commandbutton within w_gen_rep_consulta
end type
type dw_reporte_mat from u_dw within w_gen_rep_consulta
end type
type gb_1 from groupbox within w_gen_rep_consulta
end type
type gb_2 from groupbox within w_gen_rep_consulta
end type
end forward
...
global w_gen_rep_consulta w_gen_rep_consulta

on w_gen_rep_consulta.create
int iCurrent
call super::create
...
event open;call super::open;cbx_mat.checked = True

cbx_mat.triggerevent( "clicked")
...
IF cbx_mat.checked = false and cbx_esp.checked = false then
	...
	Messagebox("Aviso","Debe de indicar un tipo de Solicitud.")
ELSE
	
	IF cbx_mat.checked then
		IF rb_vencido.checked = false and rb_multa.checked = false and rb_estatus.checked = false then
			Messagebox("Aviso","Debe de indicar un tipo de Reporte de Material.")
		else
			if rb_vencido.checked then
				dw_reporte_mat.dataobject = "d_gen_rep_mat_vencido"
				dw_reporte_mat.SetTransObject(gtr_sumuia)
				li_row = dw_reporte_mat.retrieve(gi_depto,gi_area)
			elseif rb_multa.checked then
				dw_reporte_mat.dataobject = "d_gen_rep_multas"
				dw_reporte_mat.SetTransObject(gtr_sumuia)
				li_row = dw_reporte_mat.retrieve(gi_depto,gi_area)
				messagebox('row',string(li_row))
			elseif rb_estatus.checked then
				dw_reporte_mat.dataobject = "d_gen_rep_consultasol"
				dw_reporte_mat.SetTransObject(gtr_sumuia)
				li_row = dw_reporte_mat.retrieve(gi_depto,gi_area)
			end if
		end if
	elseif cbx_esp.checked then
		IF rb_vencido.checked = false and rb_estatus.checked = false then
			Messagebox("Aviso","Debe de indicar un tipo de Reporte de Espacios.")
		else
...
integer height = 2051
integer taborder = 10
string dataobject = "d_gen_rep_consultasol"
