$PBExportHeader$w_adm_cat_materiales.srw
$PBExportComments$Pantalla captura materiales admisión
forward
global type w_adm_cat_materiales from w_modelo_cap
end type
end forward
...
global w_adm_cat_materiales w_adm_cat_materiales

type variables
integer ii_gpo_mat
end variables

on w_adm_cat_materiales.create
call super::create
end on

on w_adm_cat_materiales.destroy
call super::destroy
end on
...
type dw_motivo from w_modelo_cap`dw_motivo within w_adm_cat_materiales
end type

type cb_actualiza from w_modelo_cap`cb_actualiza within w_adm_cat_materiales
end type

type cb_cerrar from w_modelo_cap`cb_cerrar within w_adm_cat_materiales
end type

type cb_eliminar from w_modelo_cap`cb_eliminar within w_adm_cat_materiales
end type

type cb_nuevo from w_modelo_cap`cb_nuevo within w_adm_cat_materiales
end type
...
type dw_cap from w_modelo_cap`dw_cap within w_adm_cat_materiales
integer width = 3547
...
if row >0 then
	...
	choose case gi_depto
		...
		case 2
			OpenSheetWithParm(w_adm_cat_inventario, envio_mat , w_ppal, 2, Original!)
	end choose
end if
end event
