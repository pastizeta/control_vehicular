...
If gtr_escolar.SQLCode <> 0 Then
	MessageBox ("Cannot Connect to Database", gtr_escolar.SQLErrText )
End If

Return gtr_escolar.SQLCode
end function
...
If gtr_escolar.SQLCode <> 0 Then
	MessageBox ("Cannot Disconnect from Database", gtr_escolar.SQLErrText )
End If
...
// gtr_escolar = create Transaction
gtr_escolar = create Transaction
...
If of_GetConnectionInfo ( ls_dbms, ls_database, ls_userid, ls_dbpass, ls_logid, ls_logpass, ls_server, ls_dbparm, ls_lock, ls_autocommit ) = 1 Then
	...
	gtr_escolar.DBMS			= ls_dbms
	gtr_escolar.Database		= ls_database
	gtr_escolar.UserID		= ls_userid
	gtr_escolar.DBPass		= ls_dbpass
	gtr_escolar.LogID			= ls_logid
	gtr_escolar.LogPass		= ls_logpass
	gtr_escolar.ServerName	= ls_server
	gtr_escolar.DBParm		= ls_dbparm
	gtr_escolar.Lock			= ls_lock
	Choose Case Lower ( ls_autocommit ) 
		Case "1", "true", "on", "yes"
			gtr_escolar.AutoCommit	= True
		Case "0", "false", "off", "no"
			gtr_escolar.AutoCommit	= False
		Case Else
			gtr_escolar.AutoCommit	= False
	End Choose
